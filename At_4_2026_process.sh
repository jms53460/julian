#!/bin/bash
#SBATCH --job-name=At_4_2026_process                                      # Job name
#SBATCH --partition=batch                                                 # Partition (queue) name
#SBATCH --ntasks=1                                                        # Single task job
#SBATCH --cpus-per-task=6                                                 # Number of cores per task
#SBATCH --mem=100gb                                                       # Total memory for job
#SBATCH --time=24:00:00                                                   # Time limit hrs:min:sec
#SBATCH --output=/scratch/jms53460/At_4_2026/At.out                      # Location of standard output file
#SBATCH --error=/scratch/jms53460/At_4_2026/At.err                       # Location of error log file
...
if [ ! -f "hisat2_out/""$file2"".bam" ]; then
	...
	hisat2 -p 6 --dta -x merged_hisat2_index -U "hisat2_out/""$file2"".fastq.gz" | samtools view -bS -> "hisat2_out/""$file2""_unsorted.bam"
...
do
    ...
    SNPsplit --conflicting -o SNPsplit --snp_file Ler_SNPs.tab "$file"
