#!/bin/bash
#SBATCH --job-name=Sol_4_2026_features_UMIs                               # Job name
#SBATCH --partition=batch                                                 # Partition (queue) name
#SBATCH --ntasks=1                                                        # Single task job
#SBATCH --cpus-per-task=6                                                 # Number of cores per task
#SBATCH --mem=300gb                                                       # Total memory for job
#SBATCH --time=24:00:00                                                   # Time limit hrs:min:sec
#SBATCH --output=/scratch/jms53460/Sol_4_2026/features_UMIs.out           # Location of standard output file
#SBATCH --error=/scratch/jms53460/Sol_4_2026/features_UMIs.err            # Location of error log file
#SBATCH --mail-user=jms53460@uga.edu                                      # Where to send mail
#SBATCH --mail-type=END,FAIL                                              # Mail events (BEGIN, END, FAIL, ALL)

cd /scratch/jms53460/Sol_4_2026/
mkdir featurecounts
mkdir bams
mkdir UMIcounts
mkdir UMIcounts_g1
