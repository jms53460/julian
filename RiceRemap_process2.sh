#!/bin/bash
#SBATCH --job-name=RiceRemap_process2                                          # Job name
#SBATCH --partition=batch                                                 # Partition (queue) name
#SBATCH --ntasks=1                                                        # Single task job
#SBATCH --cpus-per-task=6                                                 # Number of cores per task
#SBATCH --mem=200gb                                                        # Total memory for job
#SBATCH --time=24:00:00                                                   # Time limit hrs:min:sec
#SBATCH --output=/scratch/jms53460/RiceRemap/NRE1-2/Hs2.out             # Location of standard output file
#SBATCH --error=/scratch/jms53460/RiceRemap/NRE1-2/Hs2.err              # Location of error log file
#SBATCH --mail-user=jms53460@uga.edu                                      # Where to send mail
#SBATCH --mail-type=END,FAIL                                              # Mail events (BEGIN, END, FAIL, ALL)

cd /scratch/jms53460/RiceRemap/NRE1-2
...
hisat2-build NRE1-2_N-masked_Remap.fa NRE1-2_N-masked_Remap_index
...
if [ ! -f "hisat2_out/""$file2"".bam" ]; then
	...
	hisat2 -p 6 --dta -x NRE1-2_N-masked_Remap_index -U "hisat2_out/""$file2"".fastq.gz" | samtools view -bS -> "hisat2_out/""$file2""_unsorted.bam"
...
do
    ...
    SNPsplit --conflicting -o SNPsplit --snp_file NRE1-2_SNPs_Remap.tab "$file"
